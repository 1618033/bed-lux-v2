...
log = logging.getLogger("[OneShotTimer]")
...
class OneShotTimer:
    ...
    def start(self, delay_ms: int, callback: Any) -> None:
        """
    ...
    def _internal_callback(self, t):
        ...
        if self._tick_count >= self._target_ticks:
            ...
            if self._callback:
                ...
                try:
                    res = user_callback(t)
                    if res is not None and is_awaitable(res):
                        asyncio.create_task(res)  # pyright: ignore[reportGeneralTypeIssues]
                except Exception as e:
                    log.error("Error in callback for OneShotTimer._internal_callback: %s" % (e))
                    raise
                user_callback(t)
